...
impl MultiBaseConverter for Ed25519Signature {
    fn to_multibase(&self) -> String {
        ...
        utils::to_multibase_base58btc(&signature_bytes)
    }

    fn from_multibase(multibase: &str) -> Self {
        let mut signature_bytes: [u8; SIGNATURE_LENGTH] = [0; SIGNATURE_LENGTH];
        utils::from_multibase_base58btc(multibase, &mut signature_bytes); // may panic
        Ed25519Signature {
...
impl MultiBaseConverter for Ed25519SigningKey {
    fn to_multibase(&self) -> String {
        ...
        utils::to_multibase_base58btc(&public_key_bytes)
    }

    fn from_multibase(multibase: &str) -> Self {
        let mut public_key_bytes: [u8; SECRET_KEY_LENGTH] = [0; SECRET_KEY_LENGTH];
        utils::from_multibase_base58btc(multibase, &mut public_key_bytes); // may panic
        Ed25519SigningKey {
...
impl MultiBaseConverter for Ed25519VerifyingKey {
    fn to_multibase(&self) -> String {
        ...
        utils::to_multibase_base58btc(&public_key_with_prefix)
    }
    ...
    fn from_multibase(multibase: &str) -> Self {
        ...
        let mut public_key_with_prefix: [u8; PUBLIC_KEY_LENGTH + 2] = [0; PUBLIC_KEY_LENGTH + 2];
        utils::from_multibase_base58btc(multibase, &mut public_key_with_prefix);
...
impl Ed25519KeyPair {
    ...
    pub fn from(signing_key_multibase: &str) -> Self {
        ...
        utils::from_multibase_base58btc(signing_key_multibase, &mut signing_key_bytes); // may panic
